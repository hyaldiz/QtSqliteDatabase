                                 ...
                                 const QString &primaryKey,int primaryKeyIndex)
{
    ...
    if(!createQuery.exec())
        sqlError(createQuery.lastError(),"createTable exec");
    else if(!createQuery.prepare(dbvalues))
        sqlError(createQuery.lastError(),"createTable prepare");
    else {
        dbPartTable = table;
...
{
    QString errString = "Operation sender " + sender + ": sqlite error" + error.text() + "sqlite error type " + QString::number(error.type());

    if(_debugCallBack)
        _debugCallBack(errString);
    else
        qWarning() << errString;
}
...
void SQLiteDataBase::setDebugCallBack(DebugCallBack debugCallBack)
{
